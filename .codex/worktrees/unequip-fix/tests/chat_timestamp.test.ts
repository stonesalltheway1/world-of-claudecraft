...
describe('formatChatTimestamp', () => {
  it('24-hour format zero-pads hours and minutes', () => {
    expect(formatChatTimestamp(at(14, 32), '24h')).toBe('[14:32]');
    expect(formatChatTimestamp(at(9, 5), '24h')).toBe('[09:05]');
    expect(formatChatTimestamp(at(0, 0), '24h')).toBe('[00:00]');
    expect(formatChatTimestamp(at(23, 59), '24h')).toBe('[23:59]');
  });

  it('12-hour format uses AM/PM and a non-padded 12-based hour', () => {
    expect(formatChatTimestamp(at(14, 32), '12h')).toBe('[2:32 PM]');
    expect(formatChatTimestamp(at(9, 5), '12h')).toBe('[9:05 AM]');
    // midnight and noon both render as 12, not 0
    expect(formatChatTimestamp(at(0, 15), '12h')).toBe('[12:15 AM]');
    expect(formatChatTimestamp(at(12, 0), '12h')).toBe('[12:00 PM]');
  });
});
