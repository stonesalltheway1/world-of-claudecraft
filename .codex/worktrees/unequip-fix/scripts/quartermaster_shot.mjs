// Quartermaster's Consignment screenshots: the 12 new gear pieces in the bags,
...
const ALL = [
  ...
  'bristlehide_spaulders', 'sableweb_cord', 'gorraks_cleaver', 'mossy_handwraps',
];
// warrior-equippable subset, one per new armor slot + the weapon
const WORN = {
  helmet: 'roadwardens_helm', shoulder: 'reinforced_pauldrons',
  waist: 'sturdy_belt', gloves: 'roughspun_gloves', mainhand: 'gorraks_cleaver',
};
...
await page.evaluate(() => {
  ...
  if (n) { n.value = 'Quartermaster'; n.dispatchEvent(new Event('input', { bubbles: true })); }
});
...
if (box && box.width > 0) await page.screenshot({ path: 'tmp/quartermaster_character.png', clip: box });
await page.evaluate(() => window.__game.hud.toggleChar());
await wait(200);
...
if (box && box.width > 0) await page.screenshot({ path: 'tmp/quartermaster_market.png', clip: box });
