...
const SIZE = 256; // supersample; the driver downscales to the shipped size
...
function makeLights() {
  ...
  const key = new THREE.DirectionalLight(0xfff0dc, 2.4); key.position.set(2.5, 4, 3); g.add(key);
  const fill = new THREE.DirectionalLight(0x9fb6e0, 1.0); fill.position.set(-3, 1, -1.5); g.add(fill);
  const rim = new THREE.DirectionalLight(0xffffff, 1.2); rim.position.set(0, 2, -4); g.add(rim);
...
window.renderWeapon = (b64) => new Promise((resolve, reject) => {
